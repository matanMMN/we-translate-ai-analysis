...
class TranslationJobController(BaseController[TranslationJob]):
    ...
    async def create_translation_job(self,translation_job_create_schema:TranslationJobCreateSchema) -> TranslationJob:
        return await self.translation_job_service.create_translation_job(translation_job_create_schema)
    ...
    async def update_translation_job(self,translation_job_id: str, translation_job_update_schema:TranslationJobUpdateSchema) -> None:
        return await self.translation_job_service.update_translation_job(translation_job_id, translation_job_update_schema)

    @Transactional(propagation=Propagation.REQUIRED_NEW)
    async def delete_translation_job(self,translation_job_id: str) -> None:
        return await self.translation_job_service.delete_translation_job(translation_job_id)
